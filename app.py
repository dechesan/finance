...
def funds():
    ...
    if request.method == "POST":
        ...
        new_total = float(db.execute("SELECT cash FROM users WHERE id = ?",
                          session["user_id"])[0]["cash"]) + cash_adding
...
def get_portfolio():
    ...
    for symbol in symbols:
        shares = int(db.execute("SELECT SUM(shares) FROM transactions WHERE user_id = ? AND symbol=? GROUP BY symbol",
